...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _webhook_url(self) -> str:
        ...
        if host in {"0.0.0.0", "127.0.0.1", "localhost", "::"}:
            # BlueBubbles runs on the same Mac. Keep the registered URL on the
            # IPv4 listener instead of relying on localhost, which resolves to
            # ::1 first on macOS and cannot reach an IPv4-only aiohttp site.
            host = "127.0.0.1"
